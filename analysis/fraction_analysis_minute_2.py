...
def run_study(csv_path):
    ...
    # FRACTION 2: 1-2M (The Follower) 
    follower_phase = df[(df['rem'] >= 180) & (df['rem'] <= 240)].copy()
    
    price_bins = np.arange(0.30, 1.03, 0.03)
    price_labels = [f"{price_bins[i]:.2f}-{price_bins[i+1]:.2f}" for i in range(len(price_bins)-1)]
    follower_phase['price_band'] = pd.cut(follower_phase['poly_mid'], bins=price_bins, labels=price_labels)
    
    outcomes = df.groupby('market_slug')['poly_mid'].last().rename('final_price')
    grouped = df.groupby('market_slug')
    entries = follower_phase.sort_values('timestamp').groupby(['market_slug', 'price_band'], observed=False).first().dropna(subset=['poly_mid'])
    
    investment = 3.00
    sl_dist = 0.05
    tp_levels = np.arange(0.08, 0.56, 0.08) # 8c, 16c, 24c, 32c, 40c, 48c
    ...
    print("\n" + "="*120)
    print("MASTER ROI MATRIX: MINUTE 1-2 (THE FOLLOWER)")
    print("="*120)
