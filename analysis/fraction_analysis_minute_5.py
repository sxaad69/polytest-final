...
def run_study(csv_path):
    ...
    # FRACTION 5: 4-5M (Finalized) 
    final_phase = df[(df['rem'] >= 0) & (df['rem'] <= 60)].copy()
    
    price_bins = np.arange(0.30, 1.03, 0.03)
    price_labels = [f"{price_bins[i]:.2f}-{price_bins[i+1]:.2f}" for i in range(len(price_bins)-1)]
    final_phase['price_band'] = pd.cut(final_phase['poly_mid'], bins=price_bins, labels=price_labels)
    
    outcomes = df.groupby('market_slug')['poly_mid'].last().rename('final_price')
    grouped = df.groupby('market_slug')
    entries = final_phase.sort_values('timestamp').groupby(['market_slug', 'price_band'], observed=False).first().dropna(subset=['poly_mid'])
    
    investment = 3.00
    sl_dist = 0.05
    tp_levels = [0.04, 0.08, 0.12, 0.16] # Very tight TPs for the last 60s
    ...
    print("\n" + "="*120)
    print("MASTER ROI MATRIX: MINUTE 4-5 (FINALIZED)")
    print("="*120)
