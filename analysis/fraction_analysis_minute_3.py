...
def run_study(csv_path):
    ...
    # FRACTION 3: 2-3M (Momentum) 
    momentum_phase = df[(df['rem'] >= 120) & (df['rem'] <= 180)].copy()
    
    price_bins = np.arange(0.30, 1.03, 0.03)
    price_labels = [f"{price_bins[i]:.2f}-{price_bins[i+1]:.2f}" for i in range(len(price_bins)-1)]
    momentum_phase['price_band'] = pd.cut(momentum_phase['poly_mid'], bins=price_bins, labels=price_labels)
    
    outcomes = df.groupby('market_slug')['poly_mid'].last().rename('final_price')
    grouped = df.groupby('market_slug')
    entries = momentum_phase.sort_values('timestamp').groupby(['market_slug', 'price_band'], observed=False).first().dropna(subset=['poly_mid'])
    ...
    print("\n" + "="*120)
    print("MASTER ROI MATRIX: MINUTE 2-3 (MOMENTUM)")
    print("="*120)
