...
def run_simulation(csv_path):
    ...
    # Filter for the drift zone (90-120s)
    drift_zone = df[(df['rem'] >= 90) & (df['rem'] <= 120)].copy()
    
    # Define 3-cent Price Bands
    bins = np.arange(0.15, 1.05, 0.03)
    labels = [f"{bins[i]:.2f}-{bins[i+1]:.2f}" for i in range(len(bins)-1)]
    drift_zone['price_band'] = pd.cut(drift_zone['poly_mid'], bins=bins, labels=labels)
    
    # Outcomes: Get the last price for each market
    outcomes = df.groupby('market_slug')['poly_mid'].last().rename('final_price')
    
    # Process unique entries per band
    # Each market can only be "entered" once in its life for this simulation
    unique_entries = drift_zone.sort_values('timestamp').groupby(['market_slug', 'price_band'], observed=False).first().dropna(subset=['poly_mid'])
    ...
    # Group by price_band for the report
    ...
    print("\n" + "="*80)
    print("GRANULAR 3-CENT P&L MATRIX ($3.00 Entry | 90-120s Window)")
    print("="*80)
    # Filter out bands with 0 trades for readability
